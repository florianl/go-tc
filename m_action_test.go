...
func TestAction(t *testing.T) {
	tests := map[string]struct {
		...
		"mirred+cookie+index": {val: Action{
			Kind:   "mirred",
			Cookie: bytesPtr([]byte{0xAA, 0x55}), Index: uint32(42),
			Mirred: &Mirred{Parms: &MirredParam{Index: 42, Action: 1}},
		}},
		"mirred+stats": {val: Action{
			Kind:   "mirred",
			Mirred: &Mirred{Parms: &MirredParam{Index: 42, Action: 1}},
			Stats: &GenStats{
				Basic: &GenBasic{Bytes: 8, Packets: 1}, RateEst: &GenRateEst{BytePerSecond: 42, PacketPerSecond: 3},
